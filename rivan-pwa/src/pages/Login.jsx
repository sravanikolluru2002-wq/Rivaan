...
export default function Login() {
  ...
  const [props, setProps] = useState(['Villa']);

  const isSplash = screen === 'splash';
  const isLogin = screen === 'login';
  const isAgentLogin = screen === 'agent_login';
  const isAdminLogin = screen === 'admin_login';
  const isOtp = screen === 'otp';
  const isProfile = screen === 'profile';
  const isSuccess = screen === 'success';

  const statusColor = screen === 'success' ? '#eaf2e6' : '#12351d';
  const isPhoneMethod = method === 'phone';
  const isEmailMethod = !isPhoneMethod;
  const isPasswordAuth = auth === 'password';

  const tab = (on) =>
    on
      ? { flex: 1, height: '44px', border: 'none', borderRadius: '11px', background: '#fff', color: '#12351d', fontFamily: 'inherit', fontSize: '14.5px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 2px 8px rgba(18,53,29,.12)' }
      : { flex: 1, height: '44px', border: 'none', borderRadius: '11px', background: 'transparent', color: '#7c8c7e', fontFamily: 'inherit', fontSize: '14.5px', fontWeight: '600', cursor: 'pointer' };

  const phoneTabStyle = tab(isPhoneMethod);
  const emailTabStyle = tab(!isPhoneMethod);

  const pwType = showPw ? 'text' : 'password';
  const pwEye = showPw ? '🙈' : '👁';

  const PROPS = ['Apartment', 'Villa', 'Plot', 'Commercial', 'Farmhouse', 'Penthouse'];
  const propList = PROPS.map((label) => {
    const on = props.includes(label);
    const style = on
      ? { padding: '11px 16px', borderRadius: '13px', border: '1.5px solid #1a5e2e', background: '#1a5e2e', color: '#fff', fontFamily: 'inherit', fontSize: '13.5px', fontWeight: '700', cursor: 'pointer' }
      : { padding: '11px 16px', borderRadius: '13px', border: '1.5px solid #e2e8e0', background: '#fff', color: '#3d4f40', fontFamily: 'inherit', fontSize: '13.5px', fontWeight: '600', cursor: 'pointer' };
    return {
      label,
      style,
      toggle: () => {
        setProps((prev) => (prev.includes(label) ? prev.filter((x) => x !== label) : [...prev, label]));
      },
    };
  });

  const otpList = otp.map((val, idx) => ({
    val,
    idx,
    onInput: (e) => {
      ...
      if (v) {
        const nx = e.target.parentElement.querySelector(`input[data-otp="${idx + 1}"]`);
        if (nx) nx.focus();
      }
    },
    onKey: (e) => {
      if (e.key === 'Backspace' && !e.target.value) {
        const pv = e.target.parentElement.querySelector(`input[data-otp="${idx - 1}"]`);
        if (pv) pv.focus();
      }
    },
  }));
  ...
  const toggleAuthMode = () => setAuth((p) => (p === 'otp' ? 'password' : 'otp'));

  const authSwitchLabel = isPasswordAuth ? 'Prefer a one-time code?' : 'Prefer using a password?';
  const authSwitchAction = isPasswordAuth ? 'Use OTP instead' : 'Use password';
  const loginBtnLabel = isPasswordAuth ? 'Login' : 'Continue';

  const loginContinue = () => setScreen(isPasswordAuth ? 'success' : 'otp');
  const goSplash = () => setScreen('splash');
  const goLogin = () => setScreen('login');
  const goAgentLogin = () => setScreen('agent_login');
  const goAgentDash = () => navigate('/agent');
  const goAdminLogin = () => setScreen('admin_login');
  const goAdminDash = () => navigate('/admin');
  const goApp = () => navigate('/app');
  const goProfile = () => setScreen('profile');
  const goSuccess = () => setScreen('success');
  ...
  <div className="rv-phone">
    {/* status bar */}
    <div style={{position: 'absolute', top: '0', left: '0', right: '0', height: '52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 30px', fontSize: '14px', fontWeight: '700', color: statusColor, zIndex: '30'}}>
      <span>9:41</span>
